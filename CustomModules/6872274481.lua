...
local Reinject = {["Enabled"] = false}
    ...
    local AnimDisabler = {["Enabled"] = false}
    AnimDisabler = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "AnimationDisabler",
        ["HoverText"] = "Freezes your animation.",
        ["Function"] = function(callback)
            if callback then
                AnimDisabler["ToggleButton"](true)
    ...
    PurpleLighting = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
				...
				warningNotification("PurpleLighting","Tint Disabled on Rejoin.",5)
				local newsky = Instance.new("Sky", game.Lighting)
    ...
    NoRoot = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                ...
                task.wait()
                primary.Parent = char
				warningNotification("Voidware", "HumanoidRootPart Destroyed! Reset your character to disable.", 3)
            end
        end
    })
    ...
    DoublesQueue = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "DoublesQueue",
        ["HoverText"] = "Starts a queue for doubles",
        ["Function"] = function(callback)
            if callback then
        ...
        ["Function"] = function(callback)
            if callback then
                CustomMatches["ToggleButton"](false)
					...
					warningNotification("QuickCustoms", "Creating the custom match... (Bedwars Sqauds)", 6)
					task.wait(6.88)
    ...
    local Messages = {"Infinite","100","inf","120!",":/","W","900000000","inf", "Voidware", "dsc.gg/Voidware", "Vape from the Void"}
	local Indicator = {["Enabled"] = false}
    Indicator = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "Indicator",
        ["HoverText"] = "adds custom messages to the damage indicator (change in module file)",
        ["Function"] = function(callback)
            if callback then
    ...
    rainbow = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "RedSword",
        ["HoverText"] = "makes the tool your holding red. retoggle after switching tools.",
        ["Function"] = function(callback)
            if callback then
				rainbow["ToggleButton"](false)
				local rainbow = nil
    ...
    local Info = {["Enabled"] = false}
    Info = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "ConfigInfo",
        ["HoverText"] = "sends config information",
        ["Function"] = function(callback)
            if callback then
				warningNotification("Voidware", "Config created by SystemXVoid#7813 | dsc.gg/voidware", 5)
				task.wait(2)
				warningNotification("Voidware", "Config version is currently at 2.9", 5)
				task.wait(2)
				warningNotification("Voidware", "Custom Modules are loaded.", 5)
				task.wait(1)
				Info["ToggleButton"](false)
            end
        end
    })
...
if animwalk.Value == "Ud'zal" then
	...
	crasher = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				warningNotification("FPSCrashShield","Hold the shield in your hand. (DON'T PRESS IT DOWN)",5)
				task.spawn(function()
					repeat 
						task.wait(0.3) 
	...
	KillAll = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				local bow = getBow()
				if not bow and not getItem("snowball") then
					warningNotification("FunnyTPAura","Failed to toggle. No Compatiable Projectile found. (Bow, Crossbow, Snowball)",5)
					KillAll.ToggleButton(false)
					return
				end
				if bow and not getItem("arrow") and not getItem("snowball") then
					KillAll.ToggleButton(false)
					warningNotification("FunnyTPAura","Failed to toggle. Please restock on arrows or balls.",5)
					return
				end
				warningNotification("FunnyTPAura","Attempting to Teleport.. If the teleport process doesn't start, retoggle.",5) 
				task.spawn(function()
							...
							if v10.Team ~= game.Players.LocalPlayer.Team then
								while v10 and v10.Character.Humanoid.Health > 0 and v10.Character.PrimaryPart do
									...
									if KillAll.Enabled then
										if not getItem("arrow") and not getItem("snowball") and bow then KillAll.ToggleButton(false) warningNotification("FunnyTPAura","Teleporting stopped. Please restock on arrows or snowballs to continue.",5) return end
									...
									game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
										...
										repeat task.wait() until not getgenv().checked
											   if humanoid.Health < 0 then
												getgenv().checked = false
													 if KillAll.Enabled then
														warningNotification("FunnyTPAura","Died while teleporting.",5)
														KillAll.ToggleButton(false)
														return
													 end
													end
	...
	p13:Connect(function(p14)
		if ActionNotifications["Enabled"] then
			if p14.player.Name == lplr.Name then
				createwarning("Voidware", "You have destroyed a bed", 7)
			end
		end
	end)
end)
...
Client:WaitFor("EntityDeathEvent"):andThen(function(p13)
	p13:Connect(function(p14)
		if ActionNotifications["Enabled"] then
			if p14.player.Name == lplr.Name then
				createwarning("Voidware", "gg bro, you have died.", 7)
			end
		end
	end)
end)
	...
	p6:Connect(function(p7)
		if ActionNotifications["Enabled"] then
			if p7.fromEntity and p7.fromEntity == lplr.Character then
				...
				createwarning("Voidware", plr.Name.."has been clapped, too ez", 7)
			end
		end
	end)
end)

local ActionNotifications = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
	["Name"] = "ActionNotifications",
	["Function"]= function(callback) ActionNotifications["Enabled"] = callback end,
	["HoverText"] = "Sends notifications when you do stuff (destroying beds, kill, death etc.)"
})
